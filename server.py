...
def helper(folder_path):
    ...
    split_str = SERVER_FOLDER + client_id
    for file in server_files:
        true_path = folder_path.split(split_str)[1]
        print(true_path)
        true_file = true_path + file
        print(true_file)
        true_file = true_file[1:]
        print(true_file)
        client_socket.send(true_file.encode())
        print('stuck')
